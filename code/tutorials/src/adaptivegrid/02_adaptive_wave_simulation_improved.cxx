...
void step(Grid& up, const Grid& u, double dt, const Delta delta) {
    //inner points
    pfor({0, 0}, up.size(), [&](const auto& pos) {
        ...
        if (up[pos].getActiveLayer() == 1) {
            ...
            up[{i, j}] = config::a * 2 * getValue(u[{i, j}]) - config::b * getValue(up[{i, j}]) + config::c * lap + dF;
            ...
            up[pos].forAllActiveNodes([&](const Point& cell_pos, double& element) {
                ...
                element = config::a * 2 * getValue(u[{i, j}],{si,sj}) - config::b * getValue(up[{i, j}],{si,sj}) + config::c * lap;
...
void initialize(Grid& up, const Grid& u, double dt, const Delta delta) {
    step<init_config>(up,u,dt,delta);
}

void update(Grid& up, const Grid& u, double dt, const Delta delta) {
    step<update_config>(up,u,dt,delta);
}
...
void adapt(Grid& up, Grid& u) {
    ...
    pfor({0, 0}, u.size(), [&](const auto& pos) {
        ...
        double change = fabs(getValue(u[pos]) - getValue(up[pos]));
...
int main() {
    ...
    setupWave(up,{N/4,N/4},1,{N/8,N/8});
    ...
    // initialize simulation (setting up the u state)
    initialize(u,up,dt,{dx,dy});
